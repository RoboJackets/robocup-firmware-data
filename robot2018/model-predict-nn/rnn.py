...
if saved_batch_size != batch_size:
    ...
    (train_x, train_y) = pred_data.get_data(frame_delay=6, window_size=batch_size)
...
model.fit(train_x, train_y, epochs=3, batch_size=32, shuffle=True)
...
plt.plot(df_res["x"].shift(6+5)) # 6 frame ago predicted positions
